...
def append_line_in_csv(date, opened_time, closed_time, window_name):
    ...
    filename = f"/home/{os.getlogin()}/.cache/Watcher/raw_data/{date}.csv"
...
def log_creation():
    filename = f"/home/{os.getlogin()}/.cache/Watcher/raw_data/{get_date()}.csv"
    if not(os.path.isfile(filename)):
        with open(filename, 'a') as csvfile:
            csvwriter = csv.writer(csvfile, delimiter='\t')
            csvwriter.writerow([get_time(), "00:00:00", ""])
    logout_time = os.popen(f"tail -n1 {filename}").read().split("\t")[0]
    ...
    while True:
        ...
        if (y.returned_from_afk(afk, afkTimeout)):
            ...
            if os.path.exists(filename):
                opened_at = os.popen(f" tail -n1 {filename}").read().split("\t")[0]
            else:
                previous_date = os.popen("""date -d "1 day ago" '+%Y-%m-%d'""").read()[:-1]
                opened_at = (
                    os.popen(
                        f" tail -n1 ~/.cache/Watcher/raw_data/{previous_date}.csv"
                    )
                    .read()
                    .split("\t")[0]
                )
