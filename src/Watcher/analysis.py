...
def extract_data(date):
    user = os.getlogin()
    path = f"/home/{user}/.cache/Watcher/raw_data/"
...
def final_report(window_opened, time_spent):
    ...
    sorted_values = [to.convert_into_sec(y) for y in report.values()]
...
def weekly_logs(week=str(os.popen('''date +"W%V-%Y"''').read()[:-1])):
    user = os.getlogin()
    ...
    with open(filename, "w") as csvfile:
        ...
        if os.popen('''date +"W%V-%Y"''').read()[:-1] == week:
            dates = week_dates()
        else:
